...
\begin{itemize}
  ...
  GloVe vector \cite{glove}, average the embeddings for all words in the paragraph
  ...
  we apply a BiLSTM \cite{bilstm} to the word vectors, and take the concatenated
  ...
  pre-trained represetations from ELMo \cite{elmo}. We then apply a self-attentive
  classification network \cite{1708.00107} over the contextualized representations.
\end{itemize}

We use the AllenNLP Python library\footnote{\url{https://allennlp.org}}
\cite{allennlp} for implementing the neural network classifiers.
