...
We selected the ``drugs'' sub-domain as a test case, as it is the largest domain\oa{is this true?}
that has a ``legal'' and ``illegal'' sub-categories.
%\begin{description}
...
eBay is one of the largest hosting sites for retail sellers of various goods. Our corpus contains 118 item descriptions, each consisting of more than a few words.\oa{LC, what does that mean?} 
...
%In other words, a classifier who manages to distinguish such texts not relying on the content words, then the information is there, and there are distinct linguistic attributes to the legal and illegal texts. 
