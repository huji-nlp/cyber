...
We selected the ``drugs'' sub-domain as a test case, as it is the only domain
that has a ``legal'' and ``illegal'' sub-categories, and is large enough to allow quantitative analysis.
%\begin{description}
...
\subsection{Vocabulary Differences}
    ...
    These results suggest that there is no reason to think about all drug-related Onion texts as one domain, with legal and illegal texts as sub-domains, but rather to think of the three corpora as representing different domains.
    Threfore, using Onion data to characterize the differences between illegal and legal linguistic attributes is sensible. 
    It will also make more sense than comparing illegal to eBay texts, as we may assume that the data from the same corpus is more 
...
\begin{itemize}
  ...
  fully-connected layer.\oa{one layer?}
...
the performance, suggesting that the content\oa{what content? didn't follow the last sent} is mostly useful for determining
whether a page comes from Onion or eBay.
This is confirmed by the drop in accuracy when content words are dropped:
however, in this setting (drop cont.) the kernel SVM classifier performed best,
suggesting there is a non-linear relationship between the function words
found in a page and its source.

Surprisingly, the more sophisticated neural classifiers performed worse than Naive Bayes.
...
However, where only the POS of the content words is available,
the SVM classifier managed to distinguish between content and function words with quite a high accuracy.
This may suggest that the syntactic structure is different between the legal and illegal
domains, such that the distribution of grammatical categories is sufficient for distinguishing them with a reasonable accuracy.\oa{5.1 needs some editing; are these the only conclusions we can make?}
...
On a methodological note, Legal Onion and Illegal Onion are distinct enough
to be considered different domains (as distant as Legal Onion and eBay,
see \S\ref{sec:domain}). 
Therefore, Tor could be a good test-bed for working on classification of
legal and illegal texts.
