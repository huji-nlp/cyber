...
% stretch all tables to fit the 8 pages exactly (it looks more professional)
...
\subsection{Differences in Named Entities}\label{sec:ner}
    ...
    entity mentions from all the corpora.\footnote{We use all named entity types provided by spaCy (and not only ``Product'') to get a broader perspective on the differences between the domains in terms of their named entities. For example, the named entity “Peru” (of type “GPE”) appears multiple times in onion sites and is meant to imply the quality of a drug.} 
...
\\
\multicolumn{6}{l}{\bf Legal vs. Illegal Onion Drugs} \\
\hline
...
&& \multicolumn{1}{c}{\bf drop} & \multicolumn{1}{c}{\bf drop} & \multicolumn{1}{c}{\bf pos} & \multicolumn{1}{c}{\bf pos}\\
& \multicolumn{1}{c}{\bf full} & \multicolumn{1}{c}{\bf cont.} & \multicolumn{1}{c}{\bf func} & \multicolumn{1}{c}{\bf cont.} & \multicolumn{1}{c}{\bf func}\\
\multicolumn{6}{l}{\bf Legal vs. Illegal Onion Forums} \\
\hline
...
This is despite using pre-trained word embeddings,
...
vocabulary found in this domain, which is unlikely to be supported well by the pre-trained embeddings (see \S\ref{sec:ner}).
...
\section{Discussion} \label{sec:discussion}
  ...
  of text from Onion domain, compared to standard domains used for training NLP models and pre-trained word embeddings.
  %   may also be a result of the pre-trained word embeddings needing to adapt
...
%  Taken togetherThis suggests that future work on distinguishing legal and illegal text should not use a setting where Onion pages
...
This work was supported by the Israel Science Foundation (grant no. 929/17) and by the HUJI Cyber Security Research Center in conjunction with the Israel National Cyber Bureau in the Prime Minister's Office. We thank Wesam Al Nabki, Eduardo Fidalgo, Enrique Alegre, and Ivan de Paz for providing us their dataset of onion sites.
